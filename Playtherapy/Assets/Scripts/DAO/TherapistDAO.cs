...
public class TherapistDAO
{
    ...
    public static Therapist ConsultTherapist(string id_num)
    {
        if (DBConnection.dbconn != null)
        {
			...
			string sql = string.Format("SELECT * FROM therapist_therapist, auth_user WHERE therapist_therapist.user_ptr_id = auth_user.id and therapist_therapist.user_ptr_id = '{0}';", id_num);
